...
int main(int argc, char **argv)
{
	...
	for(int i=0;i<1;i++)
	{
		...
		rcv.sin_addr.s_addr = inet_addr("0.0.0.0");
		socklen_t size = sizeof(rcv);

		while (strcmp(inet_ntoa(rcv.sin_addr),argv[1])!=0 && ip.ip_ttl < 64) {
			...
			memcpy(packet, &ip, sizeof(ip));			//copy the HEADER TO THE PACKET
			memcpy((packet + 20), &icmp, 8);
			if(setsockopt(rsfd, IPPROTO_IP, IP_TTL, (char *) &ip.ip_ttl, sizeof(ip.ip_ttl)))
			perror("setsock");
			...
			done = select(FD_SETSIZE,&rfd,0,0,NULL);
			if(done) recvfrom(rsfd, recvpacket, sizeof(struct ip) + sizeof(struct icmp), 0, (struct sockaddr *)&rcv,&size);

			pf("\nReceived packet inside loop from %s:%d\n", inet_ntoa(rcv.sin_addr), ntohs(rcv.sin_port));
